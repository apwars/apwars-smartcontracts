...
contract APWarsWarMachineV2 is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private power;
     ...
     * @param finalPower The final attack power.
     ...
     * @param newPower The team power after deposit.
     ...
     * @param newPower The power after changes.
     */
    event PowerChanged(
     ...
     * @param newPower The attack after improvement.
     */
    ...
    function deposit(IAPWarsUnit _unit, uint256 _amount) public nonReentrant {
        ...
        power[team] = power[team].add(troopAttackPower).add(troopDefensePower);
    ...
    function _calculateTroopImprovement() internal {
        ...
        uint256 initialAttackePower = power[TEAM_A];
        uint256 initialDefensePower = power[TEAM_A];
        ...
        for (uint256 i = 0; i < allowedTeamTokenAddresses[TEAM_A].length; i++) {
            ...
            if (unit.getTroopImproveFactor() > 0 && balance > 0) {
                attackImprovement = power[TEAM_A]
                    .mul(unit.getTroopImproveFactor())
                    .div(ONE_HUNDRED_PERCENT)
                    .mul(balance.div(ONE));

                power[TEAM_A] = power[TEAM_A].add(attackImprovement);

                defenseImprovement = power[TEAM_A]
                    .mul(unit.getTroopImproveFactor())
                    .div(ONE_HUNDRED_PERCENT)
                    .mul(balance.div(ONE));

                power[TEAM_A] = power[TEAM_A].add(defenseImprovement);
            }
        }
        ...
        initialAttackePower = power[TEAM_B];
        initialDefensePower = power[TEAM_B];
        ...
        for (uint256 i = 0; i < allowedTeamTokenAddresses[TEAM_B].length; i++) {
            ...
            if (unit.getTroopImproveFactor() > 0 && balance > 0) {
                attackImprovement = power[TEAM_B]
                    .mul(unit.getTroopImproveFactor())
                    .div(ONE_HUNDRED_PERCENT)
                    .mul(balance.div(ONE));

                power[TEAM_B] = power[TEAM_B].add(attackImprovement);

                defenseImprovement = power[TEAM_B]
                    .mul(unit.getTroopImproveFactor())
                    .div(ONE_HUNDRED_PERCENT)
                    .mul(balance.div(ONE));

                power[TEAM_B] = power[TEAM_B].add(defenseImprovement);
            }
        }
    ...
    function _calculateLuckImpact() internal {
        uint256 initialAttackPower = power[war.attackerTeam];
        uint256 initialDefensePower = power[war.defenderTeam];
        ...
        if (war.isBadLuck) {
            war.finalPower = initialAttackPower - attackerPowerByLuck;
    ...
    function _calculateMoraleImpact() internal {
        uint256 initialAttackPower = war.finalPower;
        ...
        if (attackerMoraleImpactPerc > ONE_HUNDRED_PERCENT) {
            war.finalPower = war.finalPower.sub(
            ...
            war.finalPower = war.finalPower.add(
                attackerMoraleImpact
            );
    ...
    function _calculateLosses() internal {
        ...
        if (war.finalPower > war.finalDefensePower) {
            war.winner = war.attackerTeam;
        } else {
            war.winner = war.defenderTeam;
        }

        uint256 totalPower = war.finalPower.add(war.finalDefensePower);
    ...
    function finishFirstRound(bytes32 _externalRandomSource) public onlyOwner {
        ...
        if (war.finalPower > 0 && war.finalDefensePower > 0) {
        ...
        )
    {
        ...
        totalAttackPower = power[_team];
        totalDefensePower = power[_team];
    ...
    function withdrawPrize() public nonReentrant {
        ...
        uint256 teamTotalPower =
            isAttacker ? power[war.winner] : power[war.winner];
        uint256 userTotalPower =
            isAttacker
